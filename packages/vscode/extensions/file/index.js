...
function updateConfig() {
	...
	config = {
		...
		displayInfoOnTheRightSideOfStatusBar: configuration.get(
...
function showDetailedInfo() {
	if (info && info.prettySize) {
		...
		const table = [];
		if (info.prettySize)
			table.push({ header: 'Size', content: info.prettySize });
		if (info.gzipSize)
			table.push({ header: 'Gzipped', content: info.gzipSize });
		if (info.brotliSize)
			table.push({ header: 'Brotli', content: info.brotliSize });
		if (info.mimeType)
			table.push({ header: 'Mime type', content: info.mimeType });
		if (info.prettyDateCreated)
			table.push({ header: 'Created', content: info.prettyDateCreated });
		if (info.prettyDateChanged)
			table.push({ header: 'Changed', content: info.prettyDateChanged });
